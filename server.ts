...
۳. **تغییر الگوریتم تراکم سرور به BBR v3:**
  ...
  app.post("/api/analyze-kotlin", async (req, res) => {
    try {
      ...
      const isVpnService = code.includes("VpnService") || code.includes("TUN_MTU") || code.includes("DatagramChannel") || code.includes("ParcelFileDescriptor") || code.includes("VpnXService") || code.includes("startVpn") || code.includes("connectToServer");
...
2. **نشتی‌های احتمالی لایه شبکه (IPv6 & FD Leaks):**
   ...
   - عدم بسته‌شدن ایمن File Descriptorهای اتصال قبلی پتانسیل بروز خطای "Too many open files" در زمان قطع و وصل مکرر اینترنت را دارد.`
        ...
        const ai = getAiClient();
        const prompt = `تو یک متخصص ارشد اندروید و امنیت شبکه هستی با تخصص عمیق در VpnService اندروید، کاتلین، و پروتکل‌های VPN.

کد کاتلین زیر را با دقت کامل خط به خط بخوان و تحلیل واقعی انجام بده:

\`\`\`kotlin
${code}
\`\`\`

وظیفه‌ات اینه که کد رو واقعاً بخونی نه pattern matching انجام بدی. موارد زیر رو بررسی کن:
1. باگ‌های runtime (NullPointerException، Division by zero، loop بی‌نهایت، resource leak)
2. باگ‌های امنیتی Android VpnService (فراموش کردن protect()، نشت ترافیک، FD leak)
3. باگ‌های منطقی (متغیر تعریف نشده، thread safety، race condition)
4. مشکلات best practice کاتلین و اندروید

برای هر باگ: محل دقیق در کد، دلیل علمی مشکل، و راه حل کامل بنویس.
کد اصلاح شده کامل رو در انتها بده.
اگه کد واقعاً سالمه صادقانه بگو.

پاسخ به فارسی، تخصصی و صمیمی.`;
...
2. **عامل نوسان پهنای باند (MTU Bottleneck):**
   در متد \`generate\` بخش \`tunInboundOpt\`، مقدار \`MTU\` روی **1500** هاردکد شده است. این مقدار بسیار بزرگ است و زمانی که بسته‌ها با سربار رمزنگاری TLS همراه می‌شوند، از حد سقف مجاز شبکه اپراتورها عبور کرده و فایروال با دراپ کردن بسته‌های گسسته باعث کاهش شدید سرعت به صورت سینوسی می‌شود. تنظیم این فیلد روی **1360** ثبات شگفت‌انگیزی به پینگ و سرعت می‌دهد.`
        ...
        const ai = getAiClient();
        const prompt = `تو یک متخصص ارشد Android، امنیت شبکه، و کاتلین هستی.
کد کاتلین زیر را خط به خط با دقت واقعی بخوان و تحلیل کن:
...
وظایف تو:
۱. کد رو واقعاً بخون — هیچ فرضی نکن، فقط همین کد
۲. باگ‌های runtime شناسایی کن (NullPointer، division by zero، infinite loop، resource leak)
۳. باگ‌های Android VpnService پیدا کن (مثل فراموش کردن protect()، FD leak، thread blocking)
۴. باگ‌های منطقی بگیر (متغیر اسکوپ اشتباه، race condition، unhandled exception)
۵. برای هر باگ: شماره خط تقریبی، دلیل علمی، و راه‌حل بنویس
۶. کد کامل اصلاح‌شده در انتها بده

مهم: اگر کد سالمه صادقانه بگو "کد مشکلی ندارد". اگر باگ داره، حتی اگر compile بشه ولی runtime مشکل داشته باشه، حتماً گزارش کامل بده.
پاسخ به فارسی، تخصصی و صمیمی.`;
